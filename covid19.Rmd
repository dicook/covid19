...
```{r ne_data, eval=FALSE}
# Replaced with Monash data
...
```{r test_rate, fig.width=8,  fig.height=4, out.width="100%", eval=FALSE}
ord <- covid_ne_jhu %>% 
...
```{r monash_test_rate, fig.width=8,  fig.height=4, out.width="100%"}
...
covid_ne_jhu_smry %>% 
  select(state, date, count, deaths, tested, rate, test_rate) %>%
  arrange(desc(count)) %>%
  datatable(options = list(
  bPaginate = FALSE
))
...
```{r state_incidence, fig.width=10,  fig.height=5.5, eval=FALSE}
renderPlot({
  covid_ne_jhu %>% 
